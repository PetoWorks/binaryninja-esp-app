	...
	bool EspAppView::Init()
	{
		...
		if (archName.empty() || archName == "auto")
		{
			...
			if (detected)
			{
				archName = detected;
				m_logger->LogInfo(
					"Auto-detected architecture: %s for chip %s", archName.c_str(), m_chipAttr->chip_name);
			}
			else
			{
				m_logger->LogWarn("Unknown chip ID 0x%04x, cannot determine architecture.", m_header.chip_id);
				return false;
			}
		}
